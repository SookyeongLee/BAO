...
public class RequestDeal {
	...
	// 요청글삭제
	private ModelAndView deleteCtl(RequestBean requestBean) throws IOException {
		ModelAndView mav = new ModelAndView();
	...
	// 요청글 수정
	private ModelAndView modifyCtl(RequestBean requestBean) throws Exception  {
		ModelAndView mav = new ModelAndView();
		TransactionStatus status = tran.getTransaction(new DefaultTransactionDefinition());
		...
		if(this.updateReqDetail(requestBean)){
			...
			String jsonData = gson.toJson(this.rqCodeCheck(requestBean));
			mav.addObject("rqCode", jsonData);
			...
			mav.setViewName("RequestDeal/modify-after-view");
			tran.commit(status);
		}else {
	...
	//수정 페이지 이동
	private ModelAndView modifyFormCtl(RequestBean requestBean, BidBean bid) throws IOException {
		ModelAndView mav = new ModelAndView();
		System.out.println("들어왔다");
		System.out.println(requestBean.getRqCode());
		if(this.isBidderCode(requestBean)) {
			String jsonData = gson.toJson(this.rqCodeCheck(requestBean));
			mav.addObject("rqCode", jsonData);
			response.setContentType("text/html; charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>alert('입찰자가 존재하여 수정이 불가능합니다.'); </script>");
			out.flush();
			mav.setViewName("RequestDeal/modify-after-view");
		}else {
			String jsonData = gson.toJson(this.getReqDetail(requestBean));
			mav.addObject("rqd", jsonData);                                                                                                                     
			mav.setViewName("RequestDeal/requestModify");
	...
	private boolean convetToBoolean2(int data) {
		return data == 0 ? false: true;
	}
	private boolean isBidderCode(RequestBean requestBean) {
		return this.convetToBoolean2(dealIf.isBidderCode(requestBean));
	}
	private ArrayList<RequestBean> getReqDetail(RequestBean requestBean) {
		return dealIf.getReqDetail(requestBean);
	}
	private ArrayList<RequestBean> rqCodeCheck(RequestBean requestBean) {
		return dealIf.rqCodeCheck(requestBean);
	}
